...
class LunarLanderReachability(LunarLanderContinuous):
    ...
    def step(self, action):
        if self.before_parent_init:
            c = None  # can't be computed
        else:
            # note that l function must be computed before environment steps see reamdme for proof
            c = (-1.) * self.signed_distance()

        s, r, done, info = super(LunarLanderReachability, self).step(action)
...
class LunarLanderReach(LunarLanderContinuous):
    def __init__(self):
        ...
        self.terminal_shaping = True

        # this is the hack from above to make the ground flat
        self.np_random = RandomAlias

        super(LunarLanderReach, self).__init__()
    ...
    def cost(self):
        ...
        # landing or flying is acceptable. a max is equivalent to or
        return flying_distance, landing_distance

    def step(self, action):
        obs, r, done, info = super(LunarLanderReach, self).step(action)
        flying_distance, landing_distance = self.cost()
        constraint = -1. * np.maximum(flying_distance, landing_distance)
        info.update(dict(flying_distance=flying_distance,
