...
class ReferencePath(object):
    def __init__(self):
        self.curve_list = [(7.5, 80, 0.), (2.5, 100., 0.), (-5., 120., 0.)]
        self.period = 1200.
...
class PathTrackingEnv(gym.Env, ABC):
    ...
    def reset(self, **kwargs):
        ...
        init_v_x = np.random.uniform(0, 5, (self.num_agent,)).astype(np.float32)
    ...
    def judge_done(self, veh_state, stability_related):
        ...
        alpha_f, alpha_r, r, alpha_f_bounds, alpha_r_bounds, r_bounds = stability_related[:, 0], \
                                                                        ...
                                                                        stability_related[:, 4], \
                                                                        stability_related[:, 5]
        done = (np.abs(delta_ys) > 3) | (np.abs(delta_phis) > np.pi / 4.) | (np.abs(v_xs-EXPECTED_VS) < 2) | \
