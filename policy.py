...
from model import MLPNet, LamModel
...
class Policy4Reach(tf.Module):
    ...
    def apply_gradients(self, iteration, grads):
        ...
        fea_v_len = len(self.fea_v.trainable_weights)
        policy_len = len(self.policy.trainable_weights)

        obj_v_grad = grads[: obj_v_len]
        fea_v_grad = grads[obj_v_len : obj_v_len+fea_v_len]
        policy_grad = grads[obj_v_len+fea_v_len : obj_v_len+fea_v_len+policy_len]
        mu_grad = grads[obj_v_len+fea_v_len+policy_len:]
