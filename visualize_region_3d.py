...
def static_region(test_dir, iteration,
    ...
    flatten_fea_v = evaluator.policy_with_value.compute_fea_v(processed_obses).numpy()
    flatten_fea_v = flatten_fea_v[:, np.newaxis]
    ...
    plot_items = ['fea']
    data_dict = {'cs': flatten_cs, 'mu':flatten_mu, 'cstr': flatten_cstr, 'fea':flatten_fea_v}
    if baseline:
...
if __name__ == '__main__':
    ...
    static_region('./results/double_integrator/ApproximateReach-2021-12-12-16-13-52', 300000,
                  bound=(-6., 20., -13., 13.),
                  baseline=True) #
